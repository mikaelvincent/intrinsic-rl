...
def test_eval_suite_device_falls_back_to_cpu_when_cuda_unavailable(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import json
    import irl.experiments.evaluation as eval_module
    ...
    def _fake_evaluate_device(*, env: str, ckpt: Path, episodes: int, device: str, **kwargs) -> dict:
        _ = ckpt, kwargs
    ...
    monkeypatch.setattr(eval_module, "evaluate", _fake_evaluate_device)
    monkeypatch.setattr(torch.cuda, "is_available", lambda: False)

    runs_root = tmp_path / "runs_suite_device_fallback"
    results_dir = tmp_path / "results_suite_device_fallback"
    ...
    meta = json.loads((results_dir / "eval_meta.json").read_text(encoding="utf-8"))
