...
def test_gae_bootstrap_timeouts_and_dones():
...
def test_gae_no_leak_across_mid_truncation():
...
def test_ppo_update_stats_finite_discrete():
    torch.manual_seed(0)
    ...
    with torch.no_grad():
        dist0 = policy.distribution(torch.as_tensor(obs, dtype=torch.float32))
    ...
    stats = ppo_update(
        policy,
        value,
        {"obs": obs, "actions": actions, "old_log_probs": old_logp},
        advantages,
        value_targets,
        cfg,
        return_stats=True,
...
def test_ppo_kl_penalty_changes_policy_update():
    torch.manual_seed(0)
    rng = np.random.default_rng(0)
    ...
    _ = ppo_update(policy_a, value_a, batch, advantages, value_targets, cfg, return_stats=False)
