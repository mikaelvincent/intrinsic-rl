...
class _Transition:
    """Simple Transition implementation for intrinsic modules."""
...
def test_proposed_compute_batch_matches_sequential_compute():
    ...
    # Small capacity/depth so regions split quickly; enable gating and internal normalization.
    ...
    # Align all trainable weights so the only differences come from update order.
    ...
    # Reference: purely sequential calls to .compute over the batch.
    step_vals = []
    for i in range(B):
    ...
    # Test path: single call to compute_batch with reduction="none".
    with torch.no_grad():
    ...
    # Per-sample rewards must match closely between the two protocols.
    assert np.allclose(step_vals, batch_vals_np, atol=1e-5)

    # Region statistics (EMAs, counters, gates) should also match.
    ...
    # Aggregate diagnostics (gate_rate and component RMS) should agree.
...
def test_riac_compute_batch_matches_sequential_compute():
    ...
    # Region EMAs and counts should match
    stats_step = mod_step._stats
    stats_batch = mod_batch._stats
    assert set(stats_step.keys()) == set(stats_batch.keys())
    for rid in stats_step.keys():
    ...
    # Running RMS over LP should be essentially identical.
    assert abs(mod_step.lp_rms - mod_batch.lp_rms) < 1e-6
