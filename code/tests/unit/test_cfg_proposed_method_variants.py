...
def test_cfg_accepts_proposed_impact_only():
    ...
    assert abs(cfg.intrinsic.alpha_lp - 0.0) < 1e-12
...
def test_factory_builds_distinct_proposed_variants():
    ...
    assert abs(float(lp_only.alpha_impact) - 0.0) < 1e-12
    ...
    assert abs(float(impact_only.alpha_lp) - 0.0) < 1e-12
    ...
    nogate = create_intrinsic_module(
        ...
        alpha_impact=1.0,
        alpha_lp=0.5,
        gating_enabled=True,  # should be overridden by method label
    )
