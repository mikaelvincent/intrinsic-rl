...
* **Impact reward:**
  \[
  r_{\text{impact}} = \frac{|\phi(s_{t+1}) - \phi(s_t)|*2}{1 + N*{\text{ep}}(b(\phi(s_{t+1})))}
  \]
...
* **Learning Progress (LP):**
  \[
  \text{LP}*i = \max{0, \text{EMA}*{\text{long}} - \text{EMA}_{\text{short}}}
  \]
...
\[
r_{\text{int}}
= \alpha_{\text{impact}}\cdot \underbrace{r_{\text{impact}}}_{\text{RIDE-like}}
\;+\;
\alpha_{\text{LP}}\cdot \underbrace{\text{LP}_{i}}_{\text{R-IAC-like}}
\qquad \text{(when } \phi(s_t)\in R_i \text{)}
\]
...
* Joint loss:
  \[
  \mathcal{L}*{\text{repr}} = \alpha*{\text{fw}} \cdot \text{MSE}*{\text{fw}} + \alpha*{\text{inv}} \cdot \text{CE/NLL}
  \]
...
> **Note:** The plan is sequential; each sprint builds on previous. Steps include inputs, outputs, tests, and completion criteria.
...
**Testing & Verification:** unit (config parse; env step; PPO shapes), integration smoke run; TB curves; one checkpoint produced.
...
**Testing:** loss finiteness; predictor loss drops on fixed dataset; 50k‑step smoke runs.
...
**Outputs:** RIDE on MountainCar, Walker.
**Testing:** per‑episode counts reset; repeated state reduces reward.
...
**Outputs:** R‑IAC on MountainCar, Walker.
**Testing:** split triggers at capacity; EMA behavior; integration smoke runs.
...
2. Implement gate metrics and gating rule (§5.4.1).
3. Add hysteresis; metric: `gate_rate` (% gated).
4. Normalize/clipping pipeline; module unit tests.

**Dependencies:** Sprint 3.
**Outputs:** Proposed on MountainCar, Walker (100k steps).
**Testing:** gate flips only under criteria; hysteresis respected; gated reduces reward in random subregions.
**Completion Criteria:** Proposed runs stable; `gate_rate` non‑zero under noise; logs clean.

---

### **Sprint 5 — Codebase Refactor & Token Reduction**

**Goals**

* Split oversized modules into smaller components while preserving public APIs.
* Remove redundant/obvious comments and consolidate repeated helpers to reduce token usage.

**Steps**

1. **Shared tensor helpers:** create `irl.utils.torchops` (e.g., `_as_tensor`, `_ensure_2d`, one‑hot); refactor modules to use it.
2. **Intrinsic modules split:**

   * `irl/intrinsic/proposed.py` → `irl/intrinsic/proposed/` (`core.py`, `gating.py`, `normalize.py`), with `__init__.py` re‑exporting `Proposed`.
   * `irl/intrinsic/icm.py` → `irl/intrinsic/icm/` (`module.py`, `heads.py`, `encoder.py`) re‑exporting `ICM`, `ICMConfig`.
   * `irl/intrinsic/riac.py` → `irl/intrinsic/riac/` (`module.py`, `diagnostics.py`) re‑exporting `RIAC`.
3. **Models:** keep `PolicyNetwork`/`ValueNetwork` public API; move `mlp`/`FlattenObs` to `irl.models.layers` and re‑export `mlp` from `irl.models.networks` for backward compatibility.
4. **Docstring & comment trim:** shorten narrative docstrings to concise summaries referencing this spec; remove duplicated parameter explanations already enforced by type hints.
5. **Imports & re‑exports:** preserve existing import paths (`from irl.intrinsic import Proposed`, etc.) via `__all__` and re‑exports; add `DeprecationWarning` only for deep internal paths if changed.
6. **Duplication removal:** unify repeated small utilities; ensure consistent style across modules.
7. **Packaging:** include new subpackages in `setuptools` discovery; no dependency changes.

**Non‑functional Targets**

* Reduce the largest Python file to **≤ 250 lines**.
* Reduce aggregate source tokens by **≥ 15%** without losing functionality.
* Test runtime unchanged (±5%).

**Dependencies:** Sprints 0–4.
**Outputs:** Same behavior, smaller modules, stable public API.
**Testing**

* Unit: unchanged tests still pass; add tests for `torchops` helpers; check import shims.
* Integration: run smoke training/eval to verify parity; check serialization/ckpt unaffected.
* Static: ruff/black pass; wheel builds and installs.
  **Completion Criteria:** All tests green; import paths stable; code size targets met; no functional diffs on smoke runs.

---

### Sprint 6 — CNN Encoder & CarRacing
...
**Testing:** CNN output shape; gradient flow; 100k‑step smoke; throughput acceptable.
**Completion Criteria:** CarRacing runs without OOM; intrinsic logged.

---

### Sprint 7 — MuJoCo Set (Ant, HalfCheetah, Humanoid)
...
**Testing:** steps/sec acceptable; no rendering crashes; metrics present.
**Completion Criteria:** All three run headless; stable.

---

### Sprint 8 — Evaluation Harness & Plots
...
**Testing:** eval excludes intrinsic; deterministic with seed.
**Completion Criteria:** Reproducible plots; CSV summary complete.

---

### Sprint 9 — Robustness, Resume & Docs
...
**Testing:** resume from last ckpt; repeatability within tolerance.
