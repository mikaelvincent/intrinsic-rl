...
\paragraph{Training budgets beyond nominal convergence.}
The total environment-step budgets in Table~\ref{tab:env_suite} are intentionally over-allocated relative to the point of nominal convergence (i.e., when learning curves first plateau).
This deliberate slack allows us to analyze long-term stability of exploration behavior and intrinsic shaping---including late-training drift, regressions, or oscillations that may only emerge well after initial convergence.
...
\paragraph{Checkpointing for early-learning analysis.}
In addition to periodic online evaluation, we persist model checkpoints to disk for offline analysis and reproducibility.
Each run saves a baseline checkpoint at step 0 and then saves nine additional warmup checkpoints before the first regular checkpoint (ten checkpoints total before the regular cadence begins).
These warmup checkpoints are spaced approximately evenly within the first checkpoint interval and provide fine temporal resolution for evaluating early learning dynamics and performance.
After warmup, checkpoints are saved at a fixed environment-step interval throughout training.
