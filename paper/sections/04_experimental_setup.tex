...
We use the same intrinsic scaling coefficient $\eta$ and clipping range $r_{\max}$ for all intrinsic methods within a given environment (Table~\ref{tab:glpe_hparams}), so differences arise from the intrinsic signal itself rather than from reward-scale tuning.
GLPE (no gate) uses the same base intrinsic score as GLPE but disables the gating mechanism.

\subsection{RL backbone and training protocol}
\label{sec:experimental_setup:training}
All agents are trained with Proximal Policy Optimization (PPO; \cite{schulman-2017}) with generalized advantage estimation (GAE; \cite{schulman-2015}).
...
(ii) compute the intrinsic reward for each transition (when applicable), set intrinsic rewards to zero on environment-terminal transitions and on time-limit truncations without a final observation, and form the total reward used for advantage estimation;
...
Trainable intrinsic models are updated once per PPO iteration using the same on-policy batch used for PPO.
...
Each evaluated checkpoint is run for 20 episodes in a separate environment instance using deterministic action selection (distribution mode).
