...
We propose two intrinsic reward formulations within a shared framework: \textbf{GLPE (no gate)} and \textbf{GLPE}. Both combine feature-space impact with region-local learning progress; GLPE additionally introduces a region-specific gate that suppresses intrinsically rewarding but unproductive experience. We refer to these two variants collectively as the \emph{GLPE family}.
...
Given an embedding $z_t$, we assign it to a leaf region $\rho_t$ by routing it through the tree. Each leaf maintains a visit counter and a buffer of assigned points. When the number of assigned points to a leaf reaches a capacity $C$ and the leaf depth is below a maximum depth $D_{\max}$, the leaf is split: we choose the highest-variance coordinate that yields a non-degenerate split and split at the median value along that coordinate. When a leaf splits, one child inherits the parent region identifier (and its accumulated statistics), while the other child is assigned a new identifier whose statistics are initialized from its first assigned transition. This procedure yields a growing set of regions that adaptively refines frequently visited parts of feature space.
...
We optimize the policy using an on-policy actor--critic method with clipped policy updates and generalized advantage estimation (PPO with GAE; details in Section~\ref{sec:experimental_setup}). At each iteration we collect rollouts under the current policy, compute intrinsic rewards for each transition, and form the total rewards $r_t$ used for advantage estimation and policy/value updates.
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
and set $\eta_t=\eta\,w(p_t)$ in the augmented reward. When $p_{\mathrm{start}}$ and $p_{\mathrm{end}}$ are omitted, we hold $\eta_t=\eta$ fixed throughout training; all GLPE-family experiments reported in this paper use tapering.
